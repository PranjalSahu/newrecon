...
def load_prj_duke(breast_type, lesion, index):
    ...
    print(BINSx*BINSy, ANGLES, prj_allangle.shape, "lesion type ", lesion)
    x = []
    y = []
    ...
    #if int(lesion) == 1:
    #    print("Reading lesion projection")
    #    all_proj = np.fromfile("/media/pranjal/BackupPlus/CEDBT/CHO-DATA/WITH-LESION-PROJ/LE/proj_"+str(index)+"_3000x1504x25.raw", dtype='float32')
    #else:
    #    all_proj = np.fromfile("/media/pranjal/BackupPlus/CEDBT/CHO-DATA/NO-LESION-PROJ/LE/proj_"+str(index)+"_3000x1504x25.raw", dtype='float32')
    
    #all_proj = np.fromfile("/media/pranjal/cewitdata1/DBT-PROJ-DENOISE/NORMAL/3584x1600x25."+str(index)+".raw", dtype='float32')
    if lesion == 1:
        all_proj = np.fromfile("/media/pranjal/BackupPlus/DBT_Recons/normaldose_calci.raw", dtype='float32')
    else:
        all_proj = np.fromfile("/media/pranjal/BackupPlus/DBT_Recons/threedose_calci.raw", dtype='float32')
    print('Shape of projection is ', all_proj.shape)
    all_proj = np.reshape(all_proj, [25, 1500, 3000])
    
    print('Vol shape is ', all_proj.shape)
    for p in range(25):
        temp = all_proj[p, :, :]*0.001469#a[0, :, :]#.pixel_array.T
        if lesion == 1:
            temp = np.log(2000)-np.log(temp)
        else:
            temp = np.log(6000)-np.log(temp)
        ...
        start_angle = start_angle + 2.083333#2.008336
...
def sharpen_volume(vol):
  temp = np.reshape(vol, [60, 1200, 3000]) 
  for i in range(60):
    a = temp[i]
    a = unsharp_mask(a, radius=[5, 5], amount=sharp_amount, preserve_range=True)
    temp[i] = a
  temp = temp.flatten()
  return temp
...
#a = "/media/pranjal/newdrive1/REAL-DBT-PROJECTIONS/Pranjal-PMA-DATA/04140608/LE-L-CC-CLEAN1-500-MAE/"
#a = "/media/pranjal/newdrive1/REAL-DBT-PROJECTIONS/Pranjal-PMA-DATA/04140608/LE-L-CC/"
#a = "/media/pranjal/newdrive1/REAL-DBT-PROJECTIONS/MASS/CE-27/LE-R-CC/"
#a = "/media/pranjal/cewitdata2/HARD-CALCI-CASES/AR-27/T_PR_RAW_L-CC_DIAGNOSIS_0002/"
#a = "/media/pranjal/cewitdata2/HARD-CALCI-CASES/MC-37/T_PR_RAW_R-CC_DIAGNOSIS_0002/"
#a  = "/media/pranjal/cewitdata2/HARD-CALCI-CASES/AR-40/T_PR_RAW_R-CC_DIAGNOSIS_0002/"
#a = "/media/pranjal/cewitdata2/HARD-CALCI-CASES/MC-06/0002_proj/"
a = "/media/pranjal/newdrive1/REAL-DBT-PROJECTIONS/xiaoyu_data/LE-L-CC/"

pt = str(a)
#pt = pt.replace("dril/My Passport", "pranjal/BackupPlus")
print(pt)
projection_name = pt.split("/")[-3]+"-"+pt.split("/")[-2]
print('Projection name is ', projection_name)

if 'HE' in projection_name:
    exit(0)

if pt.split("/")[-2].split("-")[1] == "R":
    breast_type = "right"
else:
    breast_type = "left"
...
#host_prj_allangle_backup, h_angles   = load_prj_ima_all(breast_type, pt)
...
#beta_array       = np.around(beta_array, decimals=3)
print("BETA array")
    ...
    for beta in beta_array:
        ...
        for i in range(0, 1):
            ...
            for a in range(0, 5):
                ...
                #print('Copied d_est is ', temp1.shape)   
