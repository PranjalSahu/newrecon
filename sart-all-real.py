...
def load_prj_ima(breast_type):
    ...
    #proj_paths = glob.glob("/media/dril/ubuntudata/DBT_recon_data/"+projection_name+"/CE*.IMA")
    proj_paths = glob.glob("/media/dril/My Passport/REAL-DBT-PROJECTIONS/MASS/CE-20/HE-R-CC/*.IMA")
...
x_p0    = -IMGSIZx*0.5*Vsize_x#-116.25
y_p0    = -IMGSIZy*Vsize_y#-115.1
z_p0    = -30.0
...
a = np.load("/media/pranjal/BackupPlus/REAL-DBT-PROJECTIONS/ALL-SETTINGS/settings-"+str(proji)+".npy")
print(str(a))
pt = str(a)
...
beta_array       = []
beta_array.append(-0.15)
beta_array.append(-0.25)
beta_array.append(-0.35)
#np.random.seed(int(proji)*15)
#for i in range(4):
#    beta_array.append(np.random.uniform(0.01, 0.2))
...
for delta in delta_array:
    for beta in beta_array:
        ...
        #np.save('/media/pranjal/BackupPlus/REAL-DBT-PROJECTIONS/RECONS/'+projection_name+'_'+str(IMGSIZx)+'x'+str(IMGSIZy)+'x'+str(IMGSIZz)+'.'+str(i)+'_'+str(delta)+'_'+str(beta)+'_'+str(proji)+'.raw', host_est.astype('float16'))
        host_est.astype('float32').tofile('/media/pranjal/BackupPlus/REAL-DBT-PROJECTIONS/RECONS/'+projection_name+'_'+str(IMGSIZx)+'x'+str(IMGSIZy)+'x'+str(IMGSIZz)+'.'+str(i)+'_'+str(delta)+'_'+str(beta)+'_'+str(proji)+'.raw')
# In[ ]:
