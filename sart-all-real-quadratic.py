    ...
    for ind_nr_z  in range(ind_z-1, ind_z+2):
        for ind_nr_y in range(ind_y-1, ind_y+2):
            for ind_nr_x in range(ind_x-1, ind_x+2):
                ...
                priorbuf[ind_voxel] = priorbuf[ind_voxel] - diff#(1.0/distance)*diff/denominator
    return    
...
#beta_array       = [0, 0.025, 0.05, 0.075, 0.1, 0.125, 0.150, 0.175, 0.2]
beta_array = [0, 0.025, 0.05, 0.075, 1, 0.125, 0.15, 0.175, 0.2]
#beta_array.append(0)
...
for delta in delta_array:
    for beta in beta_array:
        ...
        host_est.astype('float32').tofile('/media/drilnvm/ubuntudata1/REAL-DBT-PROJECTIONS/RECONS-LINEAR-QUADRATIC/'+projection_name+'_'+str(IMGSIZx)+'x'+str(IMGSIZy)+'x'+str(IMGSIZz)+'.'+str(i)+'_'+str(delta)+'_'+str(beta)+'_'+str(proji)+'.raw')
