...
from skimage.filters import unsharp_mask, threshold_local, threshold_minimum, gaussian
import matplotlib.pyplot as plt
import numpy as np
import glob
...
def sharpen_volume(vol):
  ...
  for i in range(64):
    a = temp[i]
    a = unsharp_mask(a, radius=[3, 0.01], amount=1, preserve_range=True)
    temp[i] = a
  temp = temp.flatten()
  return temp
...
def blur_volume(vol):
	...
	for i in range(64):
		a = temp[i]
		a = gaussian(a, sigma=[1, 0.001], mode='reflect', preserve_range=True)
		temp[i] = a
	temp = temp.flatten()
	return temp
...
a = np.load("/media/pranjal/newdrive1/REAL-DBT-PROJECTIONS/ALL-SETTINGS/settings-"+str(proji)+".npy")
print(str(a))
pt = str(a)
...
for delta in delta_array:
    for beta in beta_array:
        ...
        for i in range(0, 5):
            print("Iteration ", i)
            d_est_orig = d_est.copy_to_host()
            temp1      = blur_volume(d_est_orig)
            ...
            for a in range(0, 5):
                ...
                #d_est_orig      = cuda.to_device(d_est_orig)
        ...
        save_path  = '/media/pranjal/BackupPlus/REAL-DBT-PROJECTIONS/RECONS-HUBER/'+projection_name+'_'+str(IMGSIZx)+'x'+str(IMGSIZy)+'x'+str(IMGSIZz)+'.'+str(i)+'_'+str(delta)+'_'+str(beta)+'_'+str(proji)+'_temp_3-blur-x-3-0.01.raw'
