...
load '/media/dril/ubuntudata/DBT-NEW/gan-90-projections-higher/projections/g_noi_nonoise_162.mat';
a        = g_noi_nonoise;
[x1, s1, c1] = do_recon(a, 91.6666);


load '/media/dril/ubuntudata/DBT-NEW/gan-90-projections-higher/predictions/model-sinogram32-higher-dril/22_prediction.mat';
b        = prediction;
b        = double(b);
[x2, s2, c2] = do_recon(b, 91.6666);


load '/media/dril/ubuntudata/DBT-NEW/gan-90-projections-higher/projections/g_noi_162.mat';
c        = g_noi;
c        = c(:, :, 11:35); 
...
function [xfbp, xartt, costart] = do_recon(g, orbit_angle)
     ...
     ds = 0.0255;      % in cm: detector element pixel size in the 's' direction; 
     dt = 0.0255;      % in cm: detector element pixel size in the 's' direction; 
    ...
    offset_z = (dod - (zfov/2 + d_objbottom_det-drz/2))/drz; %in pixels: offset of the volume ctr to the rotation ctr in the z direction; 
     ...
     [xartt, costart] = SART_dbt(Gtr,  g, zeros(800, 448, 320), 1, 0.9);
end
